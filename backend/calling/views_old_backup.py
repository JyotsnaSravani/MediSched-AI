...
def status_callback(request):
    ...
    # Trigger async task to process status (try Celery, fallback to synchronous)
    try:
        ...
        logger.info(f"Status callback task queued for {call_sid}")
    except Exception as celery_error:
        logger.warning(f"Celery not available, running status callback synchronously: {celery_error}")
...
def recording_callback(request):
    ...
    try:
        ...
        # Trigger transcription (try Celery, fallback to synchronous)
        from transcriptions.tasks import transcribe_call
        try:
            transcribe_call.delay(call_log.id)
            logger.info(f"Transcription task queued for call log {call_log.id}")
        except Exception as celery_error:
            logger.warning(f"Celery not available, running transcription synchronously: {celery_error}")
            # Run synchronously if Celery is not available
            transcribe_call(call_log.id)
